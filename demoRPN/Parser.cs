...
namespace demoRPN
{
    class Parser : Token
    {
        ...
        private void StackToStr(string[] StrStack, string[] Str, int i, int j)
        {
            int dem = 0;
            string str = "";
            while (dem < i)
        ...
        private void deArrO(string[] Str, Stack<BinaryTreeNode> S)       //Xóa của mảng OpS
        {
            while (o > S.Count)
                o--;
        }
        private void deArrN(string[] Str, Stack<BinaryTreeNode> S)       //Xóa của mảng NodeS
        {
            while (n > S.Count)
                n--;
        }
        ...
        public BinaryTreeNode InfixToExpessionTree()
        {
            ...
            for (int i = 0; i < token.Count(); i++)
            {
                ...
                if (IsNumber(k))
                {
                    ...
                    NodeS[n] = k; n++;       //Do có push nên ta gọi method thêm, sau đó tăng số thứ tự n lên.
                }
                else if (IsFunc(k))
                {
                ...
                else if (IsOperator(k))
                {
                    if ((i == 0) || (i > 0 && token[i - 1] == "("))
                    {
                        ...
                        else if (IsNumber(token[i + 1]))
                        {
                            ...
                            NodeS[n] = token[i] + token[i + 1];  n++; //Tương tự
                            i++;
                        }
                    }
                    else
            ...
            else
            {
                ...
                if (IsFunc(node.Value) == true)
                {
                    if (node.Value != "minus" && node.Value != "plus")
                        RPN = RPN + node.Value + " ";
                    else if (node.Value == "minus") RPN = RPN + "- ";
                    else RPN = RPN + "+ ";
                    switch (node.Value)
                    {
